		...
		if (list.Count > 0)
		{
			...
			foreach (Element item2 in list)
			{
				try
				{
					...
					{
						(item2.get_Location() as LocationCurve).set_JoinType(1, 0);
						(item2.get_Location() as LocationCurve).set_JoinType(0, 0);
					}
