...
public class WallJoin_SquareOff
{
		...
		if (list.Count > 0)
		{
			...
			foreach (Element item2 in list)
			{
				try
				{
					...
					{
						(item2.get_Location() as LocationCurve).set_JoinType(1, 2);
						(item2.get_Location() as LocationCurve).set_JoinType(0, 2);
					}
